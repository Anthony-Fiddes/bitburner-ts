  ...
  httpServer
    .createServer(function (req, res) {
      ...
      if (request.search("favicon.ico") !== -1) {
        requestedFile = fs.existsSync(requestedFile)
          ? requestedFile
          : process.argv[1].replace("webserver.js", "favicon.ico")
      }
...
if (require.main === module) {
  ...
  if (input.length < 3) {
    console.log(
      "Node Webserver usage : node webserver.js -d <WebServer Directory> -p <Port> ",
    )
    console.log(
      "                  or   node webserver.js -f <Configuration File>",
    )
...
process.on("uncaughtException", function (err) {
  console.log(
    "Was founded an error! Do you have any service running at port " +
      instance.getPort() +
      "?",
  )
  console.log(err)
